...
pub(crate) fn decoded_length(base64_len: usize) -> usize {
    ((base64_len + 3) / 4) * 3
}
...
///
/// The follow is deemed non-secret information:
///
...
pub(crate) fn decode_secret<'a>(input: &'_ [u8], output: &'a mut [u8]) -> Result<&'a [u8], Error> {
...
///
/// Returns the prefix of `output` that was written to.
pub(crate) fn decode_public<'a>(input: &'_ [u8], output: &'a mut [u8]) -> Result<&'a [u8], Error> {
...
fn decode<'a>(
    ...
    while let Some((byte, new_input)) = input.split_first() {
        input = new_input;

        let (item, pad) = match f(*byte) {
...
///
/// lo..=hi must not be 0..=255.  Callers this file have constant `lo` and `hi`,
/// and this function is private to this file.
